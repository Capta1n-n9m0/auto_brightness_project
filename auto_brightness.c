...
BMP_FILE_NEW *auto_adjust_image_new(BMP_FILE *input){
    BMP_FILE_NEW *res = copy_bmp_structure(input);

    uint8_t *image = (uint8_t *) res->imageData->data;
    ...
    for(int i = 0; i < width*height; i++){
        if(image[i*4 + 0] < r_min) r_min = image[i*4 + 0];
        if(image[i*4 + 1] < g_min) g_min = image[i*4 + 1];
        if(image[i*4 + 2] < b_min) b_min = image[i*4 + 2];
    }
    for(int i = 0; i < width*height; i++){
        if(image[i*4 + 0] > r_max) r_max = image[i*4 + 0];
        if(image[i*4 + 1] > g_max) g_max = image[i*4 + 1];
        if(image[i*4 + 2] > b_max) b_max = image[i*4 + 2];
    }
    r_max -= r_min;
    g_max -= g_min;
    b_max -= b_min;
    ...
    for(int i = 0; i < width*height; i++){
        if(r_ratio != NAN) image[i*4 + 0] = (int)round(r_ratio * image[i*4 + 0]);
        else image[i*4 + 0] = r_min;
        if(g_ratio != NAN) image[i*4 + 1] = (int)round(g_ratio * image[i*4 + 1]);
        else image[i*4 + 1] = g_min;
        if(g_ratio != NAN) image[i*4 + 2] = (int)round(b_ratio * image[i*4 + 2]);
        else image[i*4 + 2] = b_min;
    }
